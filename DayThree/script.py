...
with open("AdventOfCode2023/DayThree/input.txt", "r") as f:
    lines = []
    for line in f:
        lines.append(line)
...
def compare_lines(prev_line, curr_line, next_line):
    ...
    for number, start, end in curr_numbers:
        ...
        if symbol_in_prev_line or symbol_in_next_line or symbol_next_to_number:
...
for i in range(len(lines)):
    ...
    total_sum += compare_lines(prev_line, lines[i], next_line)
    print(f"line {i} done, total sum {total_sum}")
